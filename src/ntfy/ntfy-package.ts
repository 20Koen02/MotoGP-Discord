...
function buildBroadcastActionString(
  action: BroadcastAction & { type: "broadcast" }
): string {
...
export async function publish<T extends Config>(
  config: T
  ...
  if (config.tags && config.tags.length) {
    axiosConfig.headers.set(
      "X-Tags",
      typeof config.tags === "string" ? config.tags : config.tags.join(",")
    );
  }
...
import type {
  AxiosBasicCredentials as HTTPAuth,
